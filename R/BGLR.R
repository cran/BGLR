...
# This is reasonable as almost every design matrix generated by model-formulas is sparse
#    		X = model.matrix(attr(mf, "terms"), data=mf)
    		X = as(sparse.model.matrix(attr(mf, "terms"), data=mf),"dgCMatrix")
...
# This would generate a dense matrix out of a sparse
...
# colMeans if more efficient on sparse matrices 
...
# Here we check whether the design matrix is sparse or not. The appropiate C-function will be called
                  if(class(ETA[[j]]$X)!="dgCMatrix") {

                  ans = .Call("sample_beta", n, ETA[[j]]$p, ETA[[j]]$X, ETA[[j]]$x2, ETA[[j]]$b, e, varBj, varE, 1e-9)

                  } else { 

                      ans = .Call("sample_beta_sparse", n, ETA[[j]]$p, ETA[[j]]$X, ETA[[j]]$x2, ETA[[j]]$b, e, varBj, varE, 1e-9)
                ...
                if(ETA[[j]]$model=="BRR_windows"){
                  ...
                  if(class(ETA[[j]]$X)!="dgCMatrix") {
                   ...
                   ans = .Call("sample_beta", n, ETA[[j]]$p, ETA[[j]]$X, ETA[[j]]$x2, ETA[[j]]$b, e, ETA[[j]]$varB, varE, 1e-9)

                  } else { 

                      ans = .Call("sample_beta_sparse", n, ETA[[j]]$p, ETA[[j]]$X, ETA[[j]]$x2, ETA[[j]]$b, e, ETA[[j]]$varB, varE, 1e-9)
		   ...
		   ETA[[j]]$b = ans[[1]]
			...
			DF = ETA[[j]]$df0 + length(index)
                ...
                if (ETA[[j]]$model == "BL") {
                  ...
                  ans = .Call("sample_beta", n, ETA[[j]]$p, ETA[[j]]$X, ETA[[j]]$x2, ETA[[j]]$b, e, varBj, varE, ETA[[j]]$minAbsBeta)

                  } else { 

                      ans = .Call("sample_beta_sparse", n, ETA[[j]]$p, ETA[[j]]$X, ETA[[j]]$x2, ETA[[j]]$b, e, varBj, varE, ETA[[j]]$minAbsBeta)
		...
		if(ETA[[j]]$model %in% c("BayesB","BayesC"))
		{
			#Update marker effects
                        ...
                        if(ETA[[j]]$model=="BayesB")
                        {
                          ...
                          ans=.Call("sample_beta3_sparse",n,ETA[[j]]$p, ETA[[j]]$X, ETA[[j]]$x2, ETA[[j]]$b, ETA[[j]]$d, e, ETA[[j]]$varB, varE, 1e-9, ETA[[j]]$probIn);
                          ...
                          if(class(ETA[[j]]$X)!="dgCMatrix") {
                            ans=.Call("sample_beta3",n,ETA[[j]]$p, ETA[[j]]$X, ETA[[j]]$x2, ETA[[j]]$b, ETA[[j]]$d, e, rep(ETA[[j]]$varB,ETA[[j]]$p), varE, 1e-9, ETA[[j]]$probIn);
                              ...
                              ans=.Call("sample_beta3_sparse",n,ETA[[j]]$p, ETA[[j]]$X, ETA[[j]]$x2, ETA[[j]]$b, ETA[[j]]$d, e, rep(ETA[[j]]$varB,ETA[[j]]$p), varE, 1e-9, ETA[[j]]$probIn);
